...
function Contrant() {
    ...
    const [contrant, setcontrant] = useState([]);
    ...
    const fetchcontrant = async () => {
        axios.get("http://localhost:8005/contrat/encours", {
            params: { searchTerm: searchTerm, page: currentPage, itemsPerPage: pageSize }
        })
            .then(response => {
                setcontrant(response.data.contratEncours);
                setTotalPages(response.data.totalPages);
                Setloadings(false)
            })
            .catch(error => {
                console.error("Error fetching contrant:", error);
                Setloadings(false)
            });
    };
    ...
    const handleDelete = (itemId) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`http://localhost:8005/contrat/${itemId}`)
                    .then((response) => {
                        toast.success('Votre contrat a été supprimé.');
                        setcontrant(contrant.filter(contrant => contrant.id !== itemId));
                    })
                    .catch((error) => {
                        // En cas d'erreur lors de la suppression, afficher un message d'erreur
                        toast.error('Une erreur s\'est produite lors de la suppression de le contrant.');
                        console.error('Erreur lors de la suppression de le contrant :', error);
                    });
            }
        });
    };
        ...
        <div className='w-full'>
            ...
            <div className={`w-full overflow-y-auto  overflow-x-hidden  ${mobile3 ? 'h-[87vh]' : 'h-[79vh]'}`}>
                <div className={`h-[87vh]`}>
                    <div className=' mx-5 sm:text-2xl mt-2 text-[15px]'>Liste des contrat</div>
                                    ...
                                    {IsPrinting ? (
                                        ...
                                        <>
                                            <Whisper
                                                ...
                                                speaker={
                                                    <Popover>
                                                        Imprimer la liste des messages
                                                    </Popover>
                                                }
                                            >
                                                <div onClick={() => { SetPrintAll(true); setTimeout(() => { handlePrint() }, 0); setTimeout(() => { SetPrintAll(false); }, 0); }} className='text-green-600  block p-2 hover:bg-green-200 bg-green-100 transition-all m-0.5 cursor-pointer rounded-lg'>
                    ...
                    <div className='w-full p-2 overflow-x-auto'>
                        <div className='w-full overflow-x-auto pb-3'>
                            <table className={`border  rounded-sm overflow-hidden ${mobile ? 'w-[50em]' : 'w-full'} border-gray-200`}>
                                <thead>
                                    <tr>
                                        <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Nom complet</th>
                                        <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Objectif</th>
                                        <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Date de début</th>
                                        <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Date de fin</th>
                                        <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {contrant.map((datas, index) => (
                                        <tr key={index} className='border-b'>
                                            <td className='p-1 border-r'>{datas.nomcomplet}</td>
                                            <td className='p-1 border-r'>{datas.objectif.substring(0, 20)}</td>
                                            <td className='p-1 border-r'>{formatDate(datas.datedebut)}</td>
                                            <td className='p-1 border-r'>{formatDate(datas.datefin)}</td>
                                            <td className='p-1 border-r w-[5em]'>
                                                <div className='flex items-center justify-center w-full'>
                                                    <Whisper
                                                        ...
                                                        placement='auto'
                                                        speaker={<Popover>
                                                            Modifier le contrant
                                                        </Popover>}
                                                    >
                                                        <Link to={`/contrant/modifier/${encryptData((datas.id).toString())}`} className='text-green-600  block p-2 hover:bg-green-200 m-0.5 cursor-pointer rounded-lg'>
                                                    ...
                                                    <Whisper
                                                        ...
                                                        <Link to={`/contrant/detail/${encryptData((datas.id).toString())}`} className='text-blue-600 block p-2 hover:bg-blue-200 m-0.5 cursor-pointer rounded-lg'>
                ...
                {cahsPrintAll && <PrintAll ref={componentRef} contrant={contrant} formatDate={formatDate} />}
                <Footer />
            </div></div>
    )
}

export default Contrant
...
class PrintAll extends Component {
    render() {
        const { contrant, formatDate } = this.props;
        return (
            <div class="w-full m-1 ">
                <div className='w-full'>
                    <div className=' mx-5 text-2xl  mb-4 font-bold  text-center'>Liste des contrants</div>
                    <table className='border w-full   border-gray-200'>
                        ...
                        <tbody>
                            {contrant.map((datas, index) => (
                                <tr key={index} className='border-b'>
                                    <td className='p-1 border-r'>{datas.nomcomplet}</td>
                                    <td className='p-1 border-r'>{datas.objectif}</td>
                                    <td className='p-1 border-r'>{formatDate(datas.datedebut)} </td>
                                    <td className='p-1 border-r'>{formatDate(datas.datefin)}</td>
                                </tr>
                            ))}
