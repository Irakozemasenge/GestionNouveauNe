...
function CoordonneeAjout() {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('http://localhost:8005/coord/ajouter', data)
            .then(response => {
                toast.success('Coordonnée ajoutée avec succès');
                SetSpinnerButton(false)
                // Ajoutez ici d'autres actions à effectuer après l'ajout réussi
            })
            .catch(error => {
                if (error.response && error.response.data && error.response.data.error) {
                    ...
                    toast.error('Erreur lors de l\'ajout de la coordonnée');
                    SetSpinnerButton(false)
                }
                SetSpinnerButton(false)
            });
        ...
        <div className='w-full'>
            <div className='w-full'>
                ...
                <div className={`w-full overflow-y-auto  overflow-x-hidden  ${mobile3 ? 'h-[87vh]' : 'h-[79vh]'}`}>
                    <div className='w-full min-h-[90vh] pb-3'>
                        <div className='w-full'>
                            <div className='  h-max flex justify-center items-center'>
                                <div className="m-3 p-2  w-full rounded-xl h-max border ">
                                    <h2 className=' p-2 text-center text-[20px]  sm:text-[30px] text-[#5dca32] animeBorder'>Ajouter les coordonner physique</h2>
                                                ...
                                                <div className="flex justify-end items-center mt-4 relative pointer-events-none opacity-80">
                                                    <div className='absolute bg-transparent  pt-3  w-full h-full flex justify-center items-center z-50'>
                                                        <FadeLoader
                                                            ...
                                                            width={1}
                                                        /></div>
                                                    <input type="submit" id="send" value="Enregister" class=" transition-all bg-gray-900 rounded  cursor-pointer px-5 py-1 text-gray-600"></input>
                                                </div>
                                            </>
                                        ) : (
                                            <>
                                                ...
                                                <div className="flex justify-end w-max items-center bg-[#5dca32] text-white rounded cursor-pointer px-5 py-1">
                                                    <input type="submit" value='Enregister' id="send" ></input>
                                                </div>
                                            </>
                                        )}
                                    </form>
...
export default CoordonneeAjout
