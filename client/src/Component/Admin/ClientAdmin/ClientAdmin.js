...
function ClientAdmin() {
    ...
    const fetchclients = async () => {
        axios.get("http://localhost:8005/client", {
    ...
    const handleDelete = (itemId) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`http://localhost:8005/client/${itemId}`)
                    .then((response) => {
                        toast.success('Votre client a été supprimé.');
                        setclients(clients.filter(client => client.id !== itemId));
                    })
                    .catch((error) => {
                        // En cas d'erreur lors de la suppression, afficher un message d'erreur
                        toast.error('Une erreur s\'est produite lors de la suppression de le client.');
                        console.error('Erreur lors de la suppression de le client :', error);
                    });
            }
        });
    };
    ...
    return (
        <div className='w-full'>
            ...
            <div className={`w-full overflow-y-auto  overflow-x-hidden  ${mobile3 ? 'h-[87vh]' : 'h-[79vh]'}`}>
                <div className=' mx-5 sm:text-2xl    text-[15px]'>Liste des clients</div>
                            ...
                            <div className='flex items-center'>
                                <div>
                                    <select
                                        ...
                                        class="bg-transparent border border-[#5dca32] px-1 sm:px-2 cursor-pointer h-10 sm:text-lg first-letter:uppercase rounded-lg   block"
                                    >
                                ...
                                <div class="flex items-center mx-1  sm:mx-2 ">
                                    ...
                                    <button
                                        ...
                                        class="py-2 px-4 bg-[#00800023] hover:bg-[#00800049] text-[#5dca32] font-semibold rounded-lg"
                                    >
                                        Aller
                                    </button>
                                </div>
                ...
                <div className='w-full pb-3 min-h-[90vh] overflow-x-auto'>
                    <div className='w-full overflow-x-auto p-3'>
                        <table className={`border  rounded-sm overflow-hidden ${mobile ? 'w-[80em]' : 'w-full'} border-gray-200`}>
                            <thead>
                                <tr>
                                    <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Nom</th>
                                    <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Watsapp</th>
                                    <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>email</th>
                                    <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Montant</th>
                                    <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Service rendu</th>
                                    <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Date de service</th>
                                    <th className='border-b-2 p-2 border-r py-2 text-left border-gray-200'>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                {clients.length > 0 && clients.map((datas, index) => (
                                    <tr key={index} className='border-b'>
                                        <td className='p-1 border-r'>{datas.nomcomplet}</td>
                                        <td className='p-1 border-r'>{datas.tel}</td>
                                        <td className='p-1 border-r'>{datas.email}</td>
                                        <td className='p-1 border-r'>{datas.montant.toLocaleString()} Fbu</td>
                                        <td className='p-1 border-r'>{datas.service.length > 20 ? datas.service.slice(0, 20) + '....' : datas.service}</td>
                                        ...
                                        <td className='p-1 border-r w-[5em]'>
                                            <div className='flex items-center justify-center w-full'>
                                                <Whisper
                                                    ...
                                                    placement='auto'
                                                    speaker={<Popover>
                                                        Modifier le client <span className='font-bold italic font-serif'>{datas.nomcomplet}</span>
                                                    </Popover>}
                                                >
                                                    <Link to={`/Client/modifier/${encryptData((datas.id).toString())}`} className='text-green-600  block p-2 hover:bg-green-200 m-0.5 cursor-pointer rounded-lg'>
                                                ...
                                                <Whisper
                                                    ...
                                                    placement='auto'
                                                    speaker={<Popover>
                                                        Detail du client <span className='font-bold italic font-serif'>{datas.nomcomplet}</span>
                                                    </Popover>}>
                                                    <Link to={`/Client/detail/${encryptData((datas.id).toString())}`} className='text-blue-600 block p-2 hover:bg-blue-200 m-0.5 cursor-pointer rounded-lg'>
                                                ...
                                                <Whisper
                                                    ...
                                                    placement='auto'
                                                    speaker={<Popover>
                                                        Supprimer le client <span className='font-bold italic font-serif'>{datas.nomcomplet}</span>
                                                    </Popover>}
                                                >
                                                    <button
                        ...
                        {clients.length === 0 &&
                            <div className='w-full'>
                                <span colSpan="7" className='min-h-[60vh]   w-full flex justify-center items-center font-bold text-gray-400'>Pas des clients existé</span>
                            </div>
                        }
                    </div>
                ...
                {cahsPrintAll && <PrintAll ref={componentRef} clients={clients} formatDate={formatDate} />}
                <Footer />
            </div></div>
    )
}
...
class PrintAll extends Component {
    render() {
        const { clients, formatDate } = this.props;
        return (
            <div class="w-full m-1 ">
                <div className='w-full'>
                    <div className=' mx-5 text-2xl  mb-4 font-bold  text-center'>Liste des clients</div>
                    <table className='border w-full   border-gray-200'>
                        ...
                        <tbody>
                            {clients.map((datas, index) => (
                                <tr key={index} className='border-b'>
                                    <td className='p-1 border-r'>{datas.nomcomplet}</td>
                                    <td className='p-1 border-r'>{datas.tel}</td>
                                    <td className='p-1 border-r'>{datas.email}</td>
                                    <td className='p-1 border-r'>{datas.montant} Fbu</td>
                                    <td className='p-1 border-r'>{datas.service}</td>
                                    <td className='p-1 border-r'>{datas.createdAt && formatDate(datas.createdAt)}</td>
                                </tr>
                            ))}
                        </tbody>
