...
function TravailAddAvantage() {
    ...
    const handleSubmit = (e) => {
        ...
        if (!isValid) {
            toast.error("Veuillez remplir tous les avantages de la travail.");
            return;
        }

        setBoutLoading(true);

        axios.post(`https://speedreal.abahs-jobconnect.com/travail/addavantages/${travailId}`, avantages)
            .then((resp) => {
            ...
            .catch((err) => {
                setBoutLoading(false);
                if (err.response) {
                    console.error("Erreur lors de l'ajout des avantages à la travail :", err.response.data);
                    toast.error(err.response.data);
                    setBoutLoading(false);
                } else if (err.request) {
                    console.error("Aucune réponse du serveur lors de l'ajout des avantages à la travail :", err.request);
                    toast.error("Aucune réponse du serveur lors de l'ajout des avantages à la travail");
                    setBoutLoading(false);
                } else {
                    console.error("Erreur lors de l'ajout des avantages à la travail :", err.message);
    ...
    return (
        <div className={`w-full`}>
            ...
            <div className={`w-full overflow-y-auto overflow-x-hidden ${mobile ? 'h-[85vh]' : 'h-[80vh]'}`}>
                <div className={`w-full  overflow-hidden ${mobile ? 'min-h-[85vh]' : 'min-h-[80vh]'}`}>
                                        ...
                                        <div className="px-3 w-full">
                                            <div className="w-full flex items_center justify-between">
                                                <label className="block mt-5 font-serif text-gray-500 uppercase tracking-wide text-lg mb-1">
                                                    Ajout des avantages à la travail
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="mb-2 h-max">
                                    <div className="rounded-xl p-2 w-full">
                                        <div className="flex flex-col">
                                            <div className="mb-5">
                                                <label className="block tracking-wide text-lg mb-2">Avantages de la travail d'étude</label>
                                                {avantages.map((avantage, avantageIndex) => (
                                                    <div key={avantageIndex} className="flex items-center mt-1">
                                                        <input
                                                            ...
                                                            placeholder={`${avantageIndex + 1}. Avantage de la travail`}
                        ...
                        <div className="flex justify-end w-[95%]">
                            {boutLoading ? (
                                <label className="cursor-no-drop w-max relative mt-3 flex justify-center items-center bg-orange-950 p-2 rounded text-gray-400">
                                    <input type="submit" id="send" value='Modifier' className='pointer-events-none' disabled />
...
export default TravailAddAvantage;
