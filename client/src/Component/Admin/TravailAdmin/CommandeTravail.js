...
function CommandeTravail() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://speedreal.abahs-jobconnect.com/travail/getAllPostulations?page=${currentPage}&size=${pageSize}&search=${searchTerm}`);
                    ...
                    <div className='flex flex-wrap'>
                        {postulations.map((postulation, index) => (
                            <div className={`p-3  rounded-xl h-max ${mobile ? 'w-[98%]' : 'w-[15em] '}  m-2`}>
                                <div className='text-orange-600 font-bold text-[20px]'> {postulation.travail && postulation.travail.pays}</div>
                                <div className='mt-2'>
                                    ...
                                    <div className="font-medium">Status: <span className="font-bold text-blue-500">{postulation.status === 1 ? 'En attente' : 'Autre statut'}</span></div>
                                </div>
                                <Link to={`/travail/command/detail/${encryptData((postulation.id).toString())}`} className='w-full block hover:text-white focus:text-white text-center cursor-pointer hover:no-underline py-1.5 mt-1 bg-orange-600 text-white rounded'>
                                    Details
                                </Link>
                            </div>
                        ))}
