        ...
        <div className={`w-full`}>
            ...
            <div className={`w-full overflow-y-auto overflow-x-hidden ${mobile1 ? 'h-[85vh]' : 'h-[80vh]'}`}>
                <div className={`w-full  overflow-hidden ${mobile1 ? 'min-h-[85vh]' : 'min-h-[80vh]'}`}>
                    ...
                    <div className='flex flex-wrap'>
                        {bourses.map((bourse, index) => (
                            <div key={index} className={`p-3 rounded-xl h-max ${mobile ? 'w-[98%]' : 'w-[15em] '}  m-2`}>
                                ...
                                <Link to={`/bourse/detail/${encryptData((bourse.id).toString())}`} className='w-full block hover:no-underline focus:no-underline hover:text-white focus:text-white text-center cursor-pointer py-1.5 mt-1 bg-orange-600 text-white rounded'>
                                    Detail
                                </Link>
                            </div>
                        ))}
                    ...
                    {bourses.length === 0 && <div>Pas de bource disponible</div>}
                </div>
                <Footer />
            </div>
        </div>
