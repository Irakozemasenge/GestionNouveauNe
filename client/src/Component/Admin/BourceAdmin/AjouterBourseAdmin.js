...
function AjouterBourseAdmin() {
    ...
    const [blocs, setBlocs] = useState([
        { titre: '', paySelected: '', domaine: '', champSupplementaire: '', niveau: '', fintInscrire: '', beligibres: [{ crit: '' }], bavantages: [{ av: '' }] }
    ]);
    ...
    if (inputRefs.current.length !== blocs.length) {
        inputRefs.current = Array(blocs.length).fill({}).map((_, index) => ({
            ...
            beligibres: createRef(null),
            bavantages: createRef(null)
        }));
    }
    ...
    const validateBloc = (bloc, index) => {
        let requiredFields = ['titre', 'paySelected', 'domaine', 'niveau', 'fintInscrire', 'bavantages', 'beligibres'];
                                    ...
                                    field === 'champSupplementaire' ? 'La champ supplementaire de la domaine de la  bourse d\'étude' :
                                        field === 'niveau' ? 'Le niveau de la bourse d\'étude' :
                                            field === 'fintInscrire' ? 'La date de fin d\'inscription de la bourse d\'étude' :
                                                field === 'bavantages' ? 'Ajouter l\'avantage de bourse d\'étude' :
                                                    field === 'beligibres' ? 'Critères d\'éligibilité de la bourse d\'étude' :
    ...
    const handleAvantageChange = (e, blocIndex, AvantageIndex) => {
        const newBlocs = [...blocs];
        newBlocs[blocIndex].bavantages[AvantageIndex].av = e.target.value;
        setBlocs(newBlocs);
    };
    ...
    const handleAddAvantage = (index) => {
        const newBlocs = [...blocs];
        const lastAvantage = newBlocs[index].bavantages[newBlocs[index].bavantages.length - 1];
        ...
        const AvantageAlreadyExists = newBlocs[index].bavantages.some((critere, i) => i !== newBlocs[index].bavantages.length - 1 && critere.av.trim() === newAvantage);
        ...
        newBlocs[index].bavantages.push({ av: '' });
        setBlocs(newBlocs);
    ...
    const handleRemoveAvantage = (index, AvantageIndex) => {
        const newBlocs = [...blocs];
        newBlocs[index].bavantages.splice(AvantageIndex, 1);
        setBlocs(newBlocs);
    };
    ...
    const handleCritereChange = (e, blocIndex, critereIndex) => {
        const newBlocs = [...blocs];
        newBlocs[blocIndex].beligibres[critereIndex].crit = e.target.value;
        setBlocs(newBlocs);
    };
    ...
    const handleAddCritere = (index) => {
        const newBlocs = [...blocs];
        const lastCritere = newBlocs[index].beligibres[newBlocs[index].beligibres.length - 1];
        ...
        const critereAlreadyExists = newBlocs[index].beligibres.some((critere, i) => i !== newBlocs[index].beligibres.length - 1 && critere.crit.trim() === newCritere);
        ...
        newBlocs[index].beligibres.push({ crit: '' });
        setBlocs(newBlocs);
    ...
    const handleRemoveCritere = (index, critereIndex) => {
        const newBlocs = [...blocs];
        newBlocs[index].beligibres.splice(critereIndex, 1);
        setBlocs(newBlocs);
    };
    ...
    const Datas = blocs.map((bloc) => {
        let data = {
            ...
            beligibres: bloc.beligibres,
            bavantages: bloc.bavantages
        };
    ...
    const handleSubmit = (e) => {
        ...
        const isEmptyAvat = blocs.some((bloc) => bloc.bavantages.some((critere) => critere.av.trim() === ''));
        if (isEmptyAvat) {
        ...
        const isEmpty = blocs.some((bloc) => bloc.beligibres.some((critere) => critere.crit.trim() === ''));
        if (isEmpty) {
        ...
        setboutLoading(true)
        axios.post("https://speedreal.abahs-jobconnect.com/bourse/createBourse", Datas[0])
            .then((resp) => {
                                        ...
                                        <div className="flex flex-col">
                                            <div className="rounded-lg w-full">
                                                <div class="w-full mb-5">
                                                    <label
                                                        class="block  tracking-wide  text-[12px] sm:text-lg  mb-2"
                                                        for="grid-Title"
                                                    >
                                            ...
                                            <div className=" mb-5">
                                                <label class="block  tracking-wide  text-[12px] sm:text-lg  mb-2">Avanatage</label>
                                                {bloc.bavantages.map((avantage, avantageIndex) => (
                                                    <div key={avantageIndex} className="flex items-center  mt-1">
                                                        <input
                                                            type="text"
                                                            value={avantage.av}
                                                            onChange={(e) => handleAvantageChange(e, index, avantageIndex)}
                                                            ref={(index === blocs.length - 1 && avantageIndex === bloc.beligibres.length - 1) ? lastAvantageInputRef : null}
                                            ...
                                            <div className=" mb-5" >
                                                <label class="block  tracking-wide  text-[12px] sm:text-lg  mb-2">Critères d'éligibilité de la bourse d'etude</label>

                                                {bloc.beligibres.map((critere, critereIndex) => (
                                                    <div key={critereIndex} className="flex items-center  mt-1">
                                                        <input
                                                            type="text"
                                                            ref={(index === blocs.length - 1 && critereIndex === bloc.beligibres.length - 1) ? lastCritereInputRef : null}
                                                            value={critere.crit}
