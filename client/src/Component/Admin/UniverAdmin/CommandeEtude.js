...
function CommandeEtude() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://speedreal.abahs-jobconnect.com/etude/getAllPostulations?page=${currentPage}&size=${pageSize}&search=${searchTerm}`);
                setPostulations(response.data.postulations);
                console.log(response.data.postulations);
                    ...
                    <div className='flex flex-wrap'>
                        {postulations.map((postulation, index) => (
                            <div key={index} className="p-3 rounded-xl h-max w-full md:w-[15em] m-2">
                                <div className="text-gray-400">{postulation && postulation.titre}</div>
                                <div className="mt-2">
                                ...
                                <Link to={`/etude/command/detail/${encryptData((postulation.id).toString())}`} className="w-full block hover:text-white focus:text-white text-center hover:no-underline focus:no-underline cursor-pointer py-1.5 mt-1 bg-orange-600 text-white rounded">
                                    Details
                                </Link>
                            </div>
                        ))}
