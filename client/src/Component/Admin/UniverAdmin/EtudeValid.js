...
function EtudeValid() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://speedreal.abahs-jobconnect.com/etude/getAllPostulationValid?page=${currentPage}&size=${pageSize}&search=${searchTerm}`);
                    ...
                    <div className='flex flex-wrap'>
                        {postulations.map((postulation, index) => (
                            <div key={index} className="p-3 rounded-xl h-max w-full md:w-[15em] m-2">
                                <div className="text-gray-400">{postulation.etude && postulation.etude.titre}</div>
                                <div className="mt-2">
                                    ...
                                    <div className="font-medium">Status: <span className="font-bold">{postulation.status === 1 ? 'En attente' : 'Valide'}</span></div>
                                </div>
                                <Link to={`/etude/valid/detail/${encryptData((postulation.id).toString())}`} className="w-full block hover:text-white focus:text-white text-center cursor-pointer py-1.5 mt-1 bg-orange-600 text-white rounded">
                                    Details
                                </Link>
                            </div>
                        ))}
