...
function EtudeAdCritere() {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(`https://speedreal.abahs-jobconnect.com/etude/Addcriteres/${bourseId}`, beligibres)
            .then((resp) => {
                                            ...
                                            {beligibres.map((critere, critereIndex) => (
                                                <div key={critereIndex} className="flex items-center  mt-1">
                                                    <input
                                                        ...
                                                        className={`block w-full bg-transparent  outline-none  focus:border focus:border-fuchsia-500   border border-red rounded py-3 px-4 ${validationErrors[critereIndex].crit ? 'border-red-500' : ''}`}
                                                    />
                                                    {critereIndex <= 0 ? (
