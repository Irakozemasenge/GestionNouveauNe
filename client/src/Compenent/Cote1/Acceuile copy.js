...
function Acceuile() {
  ...
  useEffect(() => {
    axios
      .get("https://gestionnouveaune.abahs-jobconnect.online/statistiques")
      .then((response) => {
      ...
      <div className="w-full flex-wrap flex justify-around">
        ...
        <div className="w-[97%] p-2 m-1 border rounded-xl">
          <div className={`w-[97%] h-full flex justify-center`}>
            <ApexChart mobile={mobile} data={data} />
          </div>
        </div>
      </div>
    </div>
...
class ApexChart extends Component {
  ...
  updateChartData = () => {
    ...
    if (data) {
      const seriesData = [
        data.stockMedicamentCount,
        data.personnelCount,
        data.nouveauNeCount,
        data.usageMedicamentCount,
        data.consultationCount,
        data.vaccinationCount,
        data.hospitalisationCount,
      ];
      this.setState({
        series: [
          {
