  ...
  const handleDelete = (itemId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .delete(
            `https://gestionnouveaune.abahs-jobconnect.online/medicanent/stockMedicaments/${itemId}`
          )
          .then((response) => {
            toast.success("Votre medicament a été supprimé.");
            setstockMedicaments(
              stockMedicaments.filter((medicament) => medicament.id !== itemId)
            );
          })
          .catch((error) => {
            // En cas d'erreur lors de la suppression, afficher un message d'erreur
            toast.error(
              "Une erreur s'est produite lors de la suppression du medicament."
            );
            console.error(
              "Erreur lors de la suppression de le medicament :",
              error
            );
          });
      }
        ...
        <div className="h-max">
          <div className="mb-2   p-2 sm:mt-2 mt-1 w-full flex">
                  ...
                  {IsPrinting ? (
                    ...
                    <>
                      <Whisper
                        ...
                        placement="auto"
                        speaker={
                          <Popover>Imprimer la liste des Medicaments</Popover>
                        }
                      >
                        <div
                          onClick={() => {
            ...
            <div className="flex  justify-end w-max ">
              ...
              >
                <Link to="/med/ajout" className="w-max p-2 cursor-pointer">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
          ...
          <div className="w-full pb-3  overflow-y-hidden px-2 overflow-x-auto">
            <div className="w-full  pb-3">
              ...
              >
                ...
                <tbody>
                  {stockMedicaments &&
                    stockMedicaments.map((data) => {
                      return (
                        <tr className="border-b">
                          <td className="p-1 border-r">
                            {data.nom_medicament}
                          </td>
                          <td className="p-1 border-r">{data.quantite}</td>
                          <td className="p-1 border-r">
...
export default Medica2;

class PrintAll extends Component {
  render() {
    const { formatDate, stockMedicaments } = this.props;
    return (
            ...
            <tbody>
              {stockMedicaments &&
                stockMedicaments.map((data) => {
                  return (
                    <tr className="border-b">
                      <td className="p-1 border-r">{data.nom_medicament}</td>
                      <td className="p-1 border-r">{data.quantite}</td>
                      <td className="p-1 border-r">
                        {formatDate(data.date_peremption)}
                      </td>
