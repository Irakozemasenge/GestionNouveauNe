...
function DetailConsultant2() {
  ...
  const { id } = useParams();
  const consultationId = decryptData(id);
  const [data, getData] = useState({});
  useEffect(() => {
    axios
      .get(
        `https://gestionnouveaune.abahs-jobconnect.online/consultation/oneById/${consultationId}`
      )
      .then((response) => {
  ...
  return (
    <div className="flex flex-col h-[90vh] w-full overflow-hidden">
      <div className="flex w-full min-h-[82vh] overflow-x-hidden pb-5    overflow-y-auto  flex-col">
        ...
        >
          <Link to="/consult" className="m-4  w-max">
            Retour
          </Link>
        </Whisper>
        <div className="h-max">
          <div className="w-full pl-6">
            <div className="w-full pb-3">
              <div className="text-[20px] mb-5">Détail de la consultation</div>
              <div>
...
export default DetailConsultant2;
