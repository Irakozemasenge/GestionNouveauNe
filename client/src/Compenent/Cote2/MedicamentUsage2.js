...
function MedicamentUsage2() {
  ...
  const [medicament_used, setmedicament_used] = useState([]);
  ...
  const HundleClear = () => {
    ...
    setCurrentPage(1);
    setTimeout(() => {
      fetchmedicament_used();
    }, 2000);
  };

  const fetchmedicament_used = async () => {
    axios
      .get(
        "https://gestionnouveaune.abahs-jobconnect.online/api-medicament-used",
        {
          params: { search: searchTerm, page: currentPage, size: pageSize },
        }
      )
      .then((response) => {
        setmedicament_used(response.data.medicament_used);
        setTotalPages(response.data.totalPages);
        Setloadings(false);
      })
      .catch((error) => {
        console.error("Error fetching medicament_used:", error);
        Setloadings(false);
      });
  };
  useEffect(() => {
    ...
    <div className="flex flex-col h-[90vh] w-full overflow-hidden">
      ...
      {cahsPrintAll && (
        <PrintAll
          ...
          medicament_used={medicament_used}
        />
      )}
    </div>
  );
...
export default MedicamentUsage2;

class PrintAll extends Component {
  render() {
    const { formatDate, medicament_used } = this.props;
    return (
                ...
                medicament_used.map((data) => {
                  return (
                    <tr className="border-b">
                      ...
                      <td className="p-1 border-r">{data.quantite_utilisee}</td>
                      <td className="p-1 border-r">
                        {data.Personnel && data.Personnel.nom}{" "}
                        {data.Personnel && data.Personnel.prenom}
                      </td>
